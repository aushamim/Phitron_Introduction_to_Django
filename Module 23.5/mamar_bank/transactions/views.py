...
class TransferMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        acc_no = self.request.POST.get("transfer")
        amount = form.cleaned_data.get("amount")
        transfer_acc = UserBankAccount.objects.filter(account_no=acc_no).first()
        ...
        transfer_acc.save(update_fields=["balance"])

        messages.success(
            self.request,
            f'Successfully transfered {"{:,.2f}".format(float(amount))}$ from {self.request.user.username} to {transfer_acc.user.username}',
        )

        # Sent Email
        message = render_to_string(
            "transactions/transfer_email_from.html",
            {
                "sender": self.request.user,
                "receiver": transfer_acc.user,
                "amount": amount,
            },
        )
        send_email = EmailMultiAlternatives(
            "Money Transfer", "", to=[self.request.user.email]
        )
        send_email.attach_alternative(message, "text/html")
        send_email.send()
        ...
        # Received Email
        message = render_to_string(
            "transactions/transfer_email_to.html",
            {
                "sender": self.request.user,
                "receiver": transfer_acc.user,
                "amount": amount,
        ...
        send_email = EmailMultiAlternatives(
            "Money Transfer", "", to=[transfer_acc.user.email]
        )
        send_email.attach_alternative(message, "text/html")
        send_email.send()
